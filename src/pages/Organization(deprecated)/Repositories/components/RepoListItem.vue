<template>
    <Container class="py-4 border-bottom">
        <!-- <Star class="float-right mt-1 ml-1">
            <button class="btn btn-sm" :disabled="comprehensiveRepository.viewerHasStarred == undefined">
        ...
        <Name class="mb-1 wb-break-all">
            <router-link :to="repository.routerLink">{{repository.name}}</router-link>
        </Name>

        <ForkFromInfo v-if="repository.forkedFrom" class="f6 text-gray mb-1">
            Forked from 
            <router-link class="muted-link" :to="repository.forkedFrom.routerLink">
                {{repository.forkedFrom.fullName}}
            </router-link>
        </ForkFromInfo>

        <AnimatedHeightWrapper>
            <Description class="text-gray mb-2 pr-4" v-html="repository.descriptionHTML">
            </Description>
        </AnimatedHeightWrapper>

        <Topics class="flex-wrap flex-items-center f6 my-1">
            <router-link :to="`/topics/${item}`" v-for="item in repository.topics.slice(0,10)" :key="item" class="topic-tag topic-tag-link f6 my-1">{{item}}</router-link>
        </Topics>

        <TheMeta class="f6 text-gray mt-2">
            <PrimaryLanguage class="mr-3" v-if="repository.language">
                <transition name="fade" appear>
                    <span v-if="repository.languageColor" class="repo-language-color" :style="{backgroundColor: repository.languageColor || '#ccc'}"></span>
                </transition>
                {{repository.language}}
            </PrimaryLanguage>
            ...
            <ForkStatistic class="mr-3" v-if="repository.forksCount">
                ...
                {{repository.forksCount | thousands}}
            </ForkStatistic>

            <StarStatistic class="mr-3" v-if="repository.starsCount">
                ...
                {{repository.starsCount | thousands}}
            </StarStatistic>

            <openIssueCount class="mr-3" v-if="repository.openIssuesCount">
            ...
            <openIssueCount class="mr-3" v-if="repository.pullRequestsCount">
                ...
                {{repository.pullRequestsCount}}
            </openIssueCount>
            
            <UpdatedAt>
                Updated {{repository.updatedAt | getDateDiff}}
            </UpdatedAt>
        </TheMeta>
    </Container>
</template>
