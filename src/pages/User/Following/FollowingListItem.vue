<template>
    <Container class="d-flex py-4 border-bottom">
        ...
        <AvatarColumn class="col-2 mr-2">
            <router-link :to="`/${following.login}`">
                <ImgWrapper class="avatar avatar-user">
                    <img class="avatar avatar-user" height="50" width="50" :src="following.avatar_url" :alt="`@${following.login}`">
                </ImgWrapper>
            </router-link>
        ...
        <Content class="col-10">
            ...
            <router-link :to="`/${following.login}`" class="mb-1 no-underline d-flex flex-items-center flex-wrap">
                <span v-if="data.name" class="f4 link-gray-dark no-wrap">{{data.name}}</span>
                <span class="link-gray pl-1 d-inline-block">{{following.login}}</span>
            </router-link>
...
<script>
    ...
    export default {
        ...
        computed: {
            comprehensiveFollowing() {
                let extraData = this.extraDataProvided().filter(i => {
                    return i.id == this.following.node_id
                })[0] || {}
                return Object.assign({},this.following,extraData)
            },
        },
        created() {
        ...
        methods: {
            async network_getData() {
                try{
                    this.loading = true
                    let res = await authRequiredGet(this.following.url)
                    this.data = res.data
                }catch(e) {
                    console.log(e)
                }finally{
