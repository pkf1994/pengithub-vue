<template>
    <CommonLoadingWrapper class="p-3" :loading="loading">
        <IconSearchInput v-if="firstLoadedFlag && !emptyFlag" v-model="searchQuery" :search="routeWithSearchQuery"></IconSearchInput>

        <Filters v-if="firstLoadedFlag && !emptyFlag" class="d-flex flex-wrap mt-3 pb-2 border-bottom">
            <button class="btn mb-2 mr-2" @click="() => openModal('selectTypeModal')">
                <i>Type:</i>
                <span>{{$route.query.ecosystem || 'all'}}</span>   
                <span class="dropdown-caret"></span>
            </button>

            <button class="btn mr-2 mb-2"  @click="() => openModal('selectVisibilityModal')">
                <i>Visibility:</i>
                <span>{{$route.query.visibility || 'all'}}</span>   
                <span class="dropdown-caret"></span>
            </button>

            <button class="btn mb-2"  @click="() => openModal('selectSortByModal')">
                <i>Sort by:</i>
                <span>{{sortByStr}}</span>   
                <span class="dropdown-caret"></span>
            </button>
        ...
        <FilterNotice v-if="filterNoticeHTML" class="border-bottom border-gray-light py-3">
        ...
        <transition-group name="fade-group" appear>
            <PackgaeListItem class="mt-3" v-for="item in data" :key="item.name" :thePackage="item"></PackgaeListItem>
        </transition-group>
        ...
        <Modal title="Select type" ref="selectTypeModal">
            <router-link v-for="item in selectTypeModalRouterLinkData" :key="item.label" :to='item.routerLink'>
        ...
        <Modal title="Select visibility" ref="selectVisibilityModal">
            <router-link v-for="item in selectVisibilityModalRouterLinkData" :key="item.label" :to='item.routerLink'>
                <SelectMenuItem :selected="item.selected" @click.native="closeModal">
                    <span>{{item.label}}</span>    
                </SelectMenuItem>
            </router-link> 
        ...
        <Modal title="Select sort view" ref="selectSortByModal">
            <router-link v-for="item in selectSortByModalRouterLinkData" :key="item.label" :to='item.routerLink'>
                <SelectMenuItem :selected="item.selected" @click.native="closeModal">
                    <span>{{item.label}}</span>    
                </SelectMenuItem>
            </router-link> 
                ...
                return [
                    {
                        ...
                        label: "All",
                        selected: this.$route.query.ecosystem == 'all' || !this.$route.query.ecosystem
                    },
                    {
                        routerLink: `${this.$route.path}?${util_queryParse.querify({
                            ...this.$route.query,
                        ...
                        label: "npm",
                        selected: this.$route.query.ecosystem == 'npm'
                    },
                    {
                        routerLink: `${this.$route.path}?${util_queryParse.querify({
                            ...this.$route.query,
                        ...
                        label: "RubyGems",
                        selected: this.$route.query.ecosystem == 'rubygems'
                    },
                    {
                        routerLink: `${this.$route.path}?${util_queryParse.querify({
                            ...this.$route.query,
                        ...
                        label: "Maven",
                        selected: this.$route.query.ecosystem == 'maven'
                    },
                    {
                        routerLink: `${this.$route.path}?${util_queryParse.querify({
                            ...this.$route.query,
                        ...
                        label: "Docker",
                        selected: this.$route.query.ecosystem == 'docker'
                    },
                    {
                        routerLink: `${this.$route.path}?${util_queryParse.querify({
                            ...this.$route.query,
                        ...
                        label: "NuGet",
                        selected: this.$route.query.ecosystem == 'nuget'
                    },
                ]
            },
            selectVisibilityModalRouterLinkData() {
                return [
                    {
                        routerLink: `${this.$route.path}?${util_queryParse.querify({
                            ...this.$route.query,
                        ...
                        label: "All",
                        selected: this.$route.query.visibility == 'all' || !this.$route.query.visibility
                    },
                    {
                        routerLink: `${this.$route.path}?${util_queryParse.querify({
                            ...this.$route.query,
                        ...
                        label: "Private",
                        selected: this.$route.query.visibility == 'private'
                    },
                    {
                        routerLink: `${this.$route.path}?${util_queryParse.querify({
                            ...this.$route.query,
                        ...
                        label: "Public",
                        selected: this.$route.query.ecosystem == 'public'
                    }
                ]
            },
                ...
                return [
                    {
                        ...
                        selected: this.$route.query.sort_by == 'downloads_desc' || !this.$route.query.sort_by
                    },
                    {
                        routerLink: `${this.$route.path}?${util_queryParse.querify({
                            ...this.$route.query,
                            sort_by: "downloads_asc"
                        })}`,
                        label: "Least downloads",
                        selected: this.$route.query.sort_by == 'downloads_asc'
                    },
                ]
            },
            sortByStr() {
                let sort_by = this.$route.query.sort_by
                if(sort_by == "downloads_asc") return "Least downloads"
        ...
        methods: {
            async network_getData() {
                try{
                    ...
                    let url = api.API_PROXY_ORGANIZATION_PACKAGES({
            ...
            parseHTML(HTML) {
                let pattern = /<li class=".*package-list-item"[^>]*>(?:.|\r|\n)*?(<svg[^>]*>(?:.|\r|\n)*?<\/svg>)(?:.|\r|\n)*?<a.*href="(.*)">(.*)<\/a>(?:.|\r|\n)*?<span[^>]*>(.*)<\/span>(?:.|\r|\n)*?<span class="text-gray">(.*)<\/span>(?:.|\r|\n)*?<a class="link-gray" href="(.*)">(.*)<\/a>(?:.|\r|\n)*?<svg class="octicon octicon-package d-inline-block"[^>]*>.*?<\/svg>(?:\r|\n)\s*(.*)(?:\r|\n)\s*<\/span>(?:.|\r|\n)*?<svg class="octicon octicon-cloud-download d-inline-block v-align-bottom"[^>]*>.*?<\/svg>(?:\r|\n)\s*(.*)(?:\r|\n)\s*<\/span>(?:.|\r|\n)*?<\/li>/g
                let execResult
                let packages = []
                while((execResult = pattern.exec(HTML)) != null) {
                    packages.push({
                        ...
                        name: execResult[3],  
                        version: execResult[4],  
                        message: execResult[5],  
                        repositoryRouterLink: execResult[6],  
                        repository: execResult[7],  
                        dependenciesCount: execResult[8],  
                        downloadCount: execResult[9],  
                    })
                }
                ...
                let filterNoticePattern = /<div class="d-flex flex-column flex-md-row flex-md-items-center border-bottom border-gray-light py-3">(?:\r|\n)\s*<div class="flex-auto min-width-0">((?:.|\r|\n)*?)<\/div>/g
                let filterNoticeExecResult
                let filterNoticeHTML
                if((filterNoticeExecResult = filterNoticePattern.exec(HTML)) != null) {
                    filterNoticeHTML = filterNoticeExecResult[1]
                }
                this.filterNoticeHTML = filterNoticeHTML

                let paginationNextPattern = /<a[^>]*>Next<\/a>/g
                let paginationPreviousPattern = /<a[^>]*>Previous<\/a>/g
                this.pageInfo.withNext = HTML.match(paginationNextPattern) != null
                this.pageInfo.withPrev = HTML.match(paginationPreviousPattern) != null

                if(packages.length == 0 && !filterNoticeHTML) {
                    this.emptyFlag = true
                }else{
