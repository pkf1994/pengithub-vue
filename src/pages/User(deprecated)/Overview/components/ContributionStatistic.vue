                        ...
                        <div class="d-flex pt-3 pr-1">
                            <WeekItem class="d-flex flex-column" v-for="(weekItem,weekIndex) in reversedContributionCalendarWeeks" :key="weekIndex">
                                <Day class="contribution-week-item" 
                                    :class="{'not-selected':selectedDate && selectedDate != dayItem.date}" 
                                    @click="() => routerWithSpecificDate(dayItem.date)" 
                                    :date="dayItem.date" 
                                    :day="dayIndex == 0 && generateDateDay(dayItem.date)" 
                                    month="dayIndex == 0 && generateDateDay(dayItem.date) < 8 && generateMonth(dayItem.date)"
                                    v-for="(dayItem,dayIndex) in weekItem.contributionDays" 
                                    :key="dayItem.date" 
                                    :style="{backgroundColor:dayItem.color}">
                                </Day>
                            </WeekItem>
                        </div>
                    </Weeks>
