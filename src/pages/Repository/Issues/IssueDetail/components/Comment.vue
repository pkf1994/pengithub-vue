        ...
        <Inner v-if="!commentExtraDataHolder.isMinimized" class="inner bg-white">
            <Header class="px-3 header text-normal f5" :style="headerStyle">
                ...
                <HeaderInner class="py-2 flex-grow-1 position-relative" >
                    ...
                    <span class="d-inline-block text-gray btn-link" v-if="withEditHistory || data.updated_at != data.created_at">
                        edited
                        ...
                        <Popover ref="editedPopover" :popoverStyle="{left: '16px',right: '16px',top:'calc(100% + 8px)'}">
                            <div class="dropdown-header px-3 py-2 border-bottom">
                                Edited {{editHistory.data.length}} {{editHistory.data.length > 1 ? 'times' : 'time'}}
                            </div> 
                            <div class="edit-history-popover">
                                <button class="btn-link dropdown-item p-2" v-for="(item,index) in editHistory.data" :key="item.editedAt">
...
<script>
    ...
    import {authRequiredAjax,authRequiredGitHubGraphqlApiQuery} from '@/network'
