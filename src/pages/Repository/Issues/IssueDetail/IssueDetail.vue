...
<template>
  <CommonLoadingWrapper
    :loading="loading || timeline.loading || timeline.extraData.loading"
    :position="loading ? 'center' : 'corner'"
    class="px-3 bg-white flex-grow-1"
  >
    <IssueHeader
      :data="data"
    ...
    <LoadingTimeline
      v-if="data.node_id && timeline.data.length == 0 && timeline.loading"
      class="loading-timeline d-flex flex-items-center flex-justify-center"
    >
      <LoadingIconEx />
    </LoadingTimeline>
    ...
    <transition-group tag="div" appear name="fade">
      <TimelineItem
        v-for="(item, index) in handledTimelines"
        :data="item"
        :key="(item.id || '') + index"
        @quote="quoteReply"
    ...
    <LoadMore
      v-if="timeline.pageInfo.next"
      :loading="timeline.loading"
      :dataGetter="network_getTimeline"
      :hiddenItemCount="timelineRemainedCount"
    ></LoadMore>

    <transition-group tag="div" appear name="fade">
      <TimelineItem
        v-for="(item, index) in newestTimelines"
        :data="item"
        :key="(item.id || '') + index"
        @quote="quoteReply"
        @unminimize-comment="unminimizeCommentHook"
    ...
    <CommentCreatePane
      class="pt-3 mb-5 comment-create-edit-pane"
    ...
    <BottomInfoRows
      v-if="data.node_id"
      :data="Object.assign(data, extraData.data)"
    ...
    <IssueHandle
      ...
      v-if="data.id && viewerCanManageIssue()"
    >
      <LockIssueButton
        class="mt-3"
      ...
      <div
        v-if="!data.locked"
        class="text-bold link-gray-dark pt-3"
        @click="() => showModal('transferIssueModal')"
      >
        <svg
      ...
      <div
        class="text-bold link-gray-dark pt-3"
        @click="() => showModal('deleteIssueModal')"
      >
        <svg
    ...
    <Modal
      ...
      ref="transferIssueModal"
      :modalStyle="{ height: '80vh' }"
      @show="network_getAvailableTransferTargetRepositories"
    >
      <div
        v-if="transferIssueModal.availableRepositories.loading"
        class="flex-row-center height-full"
      >
      ...
      <div style="overflowy: auto">
        <transition-group name="fade-group" appear>
          <SelectMenuItem
            v-for="item in filteredTransferTargetRepositories"
            class="border-bottom"
            :key="item.id"
          ...
          >
            <div class="flex-grow-1">
              <span>
                <svg
                  v-if="item.isPrivate"
                  class="octicon octicon-lock float-right mr-3"
                  viewBox="0 0 16 16"
                  version="1.1"
                  width="16"
                  height="16"
      ...
      <div
        class="Box-footer bg-white border-top modal-footer"
        v-if="!transferIssueModal.availableRepositories.loading"
      >
        <button
          class="btn btn-block"
          :disabled="
            transferIssueModal.loading || !transferIssueModal.selectedRepository
          "
          @click="network_transferIssue"
        >
    ...
    >
      <div class="d-flex flex-column p-3 flex-items-center bg-white">
        <svg
          class="octicon octicon-circle-slash text-red ml-1"
          height="40"
          width="40"
          viewBox="0 0 24 24"
          version="1.1"
      ...
      <div class="Box-footer bg-white border-top modal-footer">
        <button
          class="btn text-red btn-block"
          :disabled="deleteIssueModal.loading"
          @click="network_deleteIssue"
        >
          {{
            deleteIssueModal.loading
              ? "Deleting issue..."
              : "Delete this issue."
          }}
        </button>
...
export default {
  ...
  computed: {
    filteredTransferTargetRepositories() {
      ...
      return this.transferIssueModal.availableRepositories.data.filter((i) => {
        return (
          i.hasIssuesEnabled &&
          i.name != this.$route.params.repo &&
          (i.name.toLowerCase().indexOf(searchQuery) != -1 ||
            (i.description &&
              i.description.toLowerCase().indexOf(searchQuery) != -1))
        );
      });
    },
  },
  ...
  methods: {
    ...
    async network_getIssueRest() {
      try {
        this.loading = true;
        let cancelToken = this.cancelAndUpdateAxiosCancelTokenSource(
          this.$options.name
        );

        let url_issue = api.API_ISSUE(this.$route.params);
        let res_issue = await authRequiredGet(url_issue, {
          cancelToken,
          headers: {
        ...
        if (
          res_issue.data.url.replace("https://api.github.com/repos", "") !=
          this.$route.path
        ) {
          this.$router.replace(
            res_issue.data.url.replace("https://api.github.com/repos", "")
    ...
    async network_getIssueExtraData() {
      try {
        ...
        let cancelToken = this.cancelAndUpdateAxiosCancelTokenSource(
          this.$options.name + " get_extra_data"
        );
      ...
      try {
        ...
        let res = await authRequiredGitHubGraphqlApiQuery(
          graphql.GRAPHQL_ISSUE_TIMELINE_COUNT,
          {
            ...
            cancelToken: this.cancelAndUpdateAxiosCancelTokenSource(
              this.$options.name + "_timeline_graphql_count"
            ),
          }
        );
        try {
          this.timeline.graphqlCount.data =
            res.data.data.repository.issue.timelineItems.totalCount;
        } catch (e) {
    ...
    async network_getAvailableTransferTargetRepositories() {
      ...
      try {
        ...
        let cancelToken = this.cancelAndUpdateAxiosCancelTokenSource(
          this.$options.name + " get_available_transfer_target_repositories"
        );
    ...
    async network_transferIssue() {
      ...
      try {
        ...
        let cancelToken = this.cancelAndUpdateAxiosCancelTokenSource(
          this.$options.name + " transfer_issue"
        );
        let res = await authRequiredGitHubGraphqlApiQuery(
    ...
    async network_deleteIssue() {
      ...
      try {
        ...
        let cancelToken = this.cancelAndUpdateAxiosCancelTokenSource(
          this.$options.name + " delete_issue"
        );
        let res = await authRequiredGitHubGraphqlApiQuery(
        ...
        this.handleGraphqlError(res);
        this.$router.replace(
          `/${this.owner}/${this.repo}/issues?delete=${this.data.id}`
        );
        this.topNoticeShow(
          "repository",
