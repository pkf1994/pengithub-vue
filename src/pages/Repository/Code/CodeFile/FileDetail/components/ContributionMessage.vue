        ...
        <template v-slot:footer>
            <Contributors class="py-2 px-3 d-flex flex-items-center flex-auto f6 border-bottom-0 flex-wrap">
                <span class="btn-link mr-2">
                    <strong>{{contributors().length}}</strong> {{contributors().length > 1 ? 'contributors' : 'contributor'}}
                </span>
                <span>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            committedDate() {
                return util_dateFormat.dateFormat('dd zzz yyyy',new Date(this.lastCommit().committedDate))
            }
        },
        components: {
            ComplexBubble,
