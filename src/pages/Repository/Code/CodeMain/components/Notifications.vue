    ...
    <ComplexBubble :loading="!codeBasicInfo().id">
        ...
        <Content class="bubble-content p-0">
           <button class="list-item" v-for="item in notificationOptions" :key="item.title">
               <svg v-if="item.viewerSubscription === codeBasicInfo().viewerSubscription" class="v-align-text-bottom d-inline-block" fill="currentColor" viewBox="0 0 12 16" version="1.1" width="12" height="16" aria-hidden="true"><path fill-rule="evenodd" d="M12 5l-8 8-4-4 1.5-1.5L4 10l6.5-6.5L12 5z"></path></svg> 
