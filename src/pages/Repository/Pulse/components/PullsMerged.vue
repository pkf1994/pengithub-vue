    ...
    <ComplexBubble :loading="data.length == 0 && pullsMerged().loading" 
        ...
        <template v-slot:title>
            <Title  class="bubble-title" style="font-weight: 700">
                ...
                {{pullsMerged().totalCount}} Pull Requests merged
            </Title>   
        </template>
        ...
        <template v-slot:footer>
            <LoadMore v-if="pullsMerged().pageInfo.hasNextPage" @click="() => getMoreData(loadingMore)" :class="{'text-gray':loadingMore}" class="text-center p-3 text-blue">
...
<script>
    ...
    export default {
        ...
        computed: {
           data() {
                let data = []
                this.pullsMerged().data.forEach(item => {
                    if(item.id) data.push(item)
                })
                return data
           },
           loadingMore() {
               return this.pullsMerged().loading && this.pullsMerged().data.length !== 0
           }
        },
        methods: {
             dateFormat(dataStr) {
