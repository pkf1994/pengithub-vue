        ...
        <AnimatedHeightWrapper :stretch="stretch">
              <DiffView  class="diff-view">
                <div class="d-inline-block">
                    <CodeLine v-for="(item,index) in diffHunkEntries" :key="index" class="flex width-full" :data-type="lazyLoadedLines.indexOf(item.additionLineIndex) > -1 ? 'lazyLoaded' :item.type">
                        <BlobNum    v-if="item.type !== 'hunk'"
                                    class="blob-num" 
                                    :data-line-number="item.deletionLineIndex === 0 ? '...' : item.deletionLineIndex" 
                                    :class="{'blob-num-lazy-loaded':lazyLoadedLines.indexOf(item.additionLineIndex) > -1,'blob-num-addition':item.type === 'addition','blob-num-deletion':item.type === 'deletion','blob-num-context':item.type === 'context','blob-num-hunk':index === 0 && diffHunkEntries.length === 0 || (item.deletionStartLineIndex === item.deletionLineIndex && item.additionStartLineIndex === item.additionLineIndex)}"></BlobNum>
                        <BlobNum    class="blob-num" 
                                    v-if="item.type !== 'hunk'"
                                    :data-line-number="item.additionLineIndex === 0 ? '...' : item.additionLineIndex" 
                                    :class="{'blob-num-lazy-loaded':lazyLoadedLines.indexOf(item.additionLineIndex) > -1,'blob-num-addition':item.type === 'addition','blob-num-deletion':item.type === 'deletion','blob-num-context':item.type === 'context','blob-num-hunk':index === 0 && diffHunkEntries.length === 0 || (item.deletionStartLineIndex === item.deletionLineIndex && item.additionStartLineIndex === item.additionLineIndex)}"></BlobNum>
                        
                        <div class="stretch-btn" v-else-if="item.type === 'hunk' && index > 0 && index < diffHunkEntries.length && (diffHunkEntries[index + 1].additionLineIndex - diffHunkEntries[index - 1].additionLineIndex > 21)">
                        ...
                        <TypeMark   :data-type="item.type" 
                                    ...
                                    :class="{'blob-code-lazy-loaded':lazyLoadedLines.indexOf(item.additionLineIndex) > -1,'blob-code-addition':item.type === 'addition','blob-code-deletion':item.type === 'deletion','blob-code-context':item.type === 'context','blob-code-hunk':index === 0 && diffHunkEntries.length === 0 || (item.deletionStartLineIndex === item.deletionLineIndex && item.additionStartLineIndex === item.additionLineIndex)}">
                            
                        </TypeMark>
                        <BlobCode   class="blob-code" 
                                    :class="{'blob-code-lazy-loaded':lazyLoadedLines.indexOf(item.additionLineIndex) > -1,'blob-code-addition':item.type === 'addition','blob-code-deletion':item.type === 'deletion','blob-code-context':item.type === 'context','blob-code-hunk':index === 0 && diffHunkEntries.length === 0 || (item.deletionStartLineIndex === item.deletionLineIndex && item.additionStartLineIndex === item.additionLineIndex)}">{{item.code.replace(/^\+/," ").replace(/^-/," ")}}</BlobCode>
                    </CodeLine>
                </div>
            </DiffView>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            showHiddenCode(hunkLine,stretchDownward,stretchable) {
                ...
                if(stretchDownward) {
                    ...
                    let fileContentRawToShow = ` ${fileContentRawToShowArr.join('\n ')}`

                    this.patch = this.patch.replace(hunkLine.code,fileContentRawToShow)
                    ...
                    if(entiesToShow.length > 20) {
                        this.patch = this.patch.split(hunkLine.code)[0] + fileContentRawToShow + '\n' + hunkLine.code + this.patch.split(hunkLine.code)[1]
                    }else{
