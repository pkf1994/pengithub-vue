  ...
  <Container class="the-container">
    <Header class="header d-flex flex-column">
      <Info class="info flex-auto min-width-0 mb-md-0 mb-2">
        ...
        <svg
          v-if="clippyFlag"
          class="octicon octicon-clippy d-inline-block mx-1"
          viewBox="0 0 14 16"
          version="1.1"
          width="14"
          height="16"
        ...
        <svg
          v-else
          class="octicon octicon-check js-clipboard-check-icon mx-1 d-inline-block text-green d-none"
          viewBox="0 0 12 16"
          version="1.1"
          width="12"
          height="16"
      ...
      <Actions
        class="actions pt-0 mb-2 flex-shrink-0 text-right position-relative"
        @click="openPopover"
      >
        <svg
          aria-label="Show options"
          class="octicon octicon-kebab-horizontal mx-2"
          viewBox="0 0 13 16"
          version="1.1"
          width="13"
          height="16"
        ...
        <Popover
          ref="popover"
          class="f5 py-2"
          :popoverStyle="{ right: '-4px', top: '25px' }"
          style="width: 185px"
        >
          <button
            @click.stop="triggerShowComments"
            class="pl-5 btn-link py-2 dropdown-item"
          >
            <span
              v-if="showComments"
    ...
    <DiffView v-if="stretch" class="diff-view" style="overflow-x: auto">
      <div class="d-inline-block" style="min-width: 100%">
        <CodeLine
          v-for="(item, index) in diffHunkEntries"
          :key="`${index}_${item.type}_${item.code}`"
          class="width-full"
          :data-type="
            lazyLoadedLines.indexOf(item.additionLineIndex) > -1
              ? 'lazyLoaded'
              : item.type
          "
        >
          <div class="d-flex">
            <BlobNum
              ...
              class="blob-num"
              :data-line-number="
                item.deletionLineIndex <= 0 ? '...' : item.deletionLineIndex
              "
              :class="{
            ...
            <BlobNum
              ...
              v-if="item.type !== 'hunk' || item.additionLineIndex == 0"
              :data-line-number="
                item.additionLineIndex <= 0 ? '...' : item.additionLineIndex
              "
              :class="{
                'blob-num-lazy-loaded':
                  lazyLoadedLines.indexOf(item.additionLineIndex) > -1,
            ...
            <div
              ...
              v-else-if="
                item.type === 'hunk' &&
                item.additionLineIndex != 0 &&
                index > 0 &&
                index < diffHunkEntries.length - 1 &&
                diffHunkEntries[index + 1].additionLineIndex -
                  diffHunkEntries[index - 1].additionLineIndex >
                  21
              "
            >
                ...
                @click="
                  () =>
                    showHiddenCode(
                      item,
                      false,
                      item.additionLineIndex === item.additionStartLineIndex &&
                        item.additionLineIndex !== 0
                    )
                "
            ...
            >
              ...
              <svg
                v-else-if="index === diffHunkEntries.length - 1"
                class="octicon octicon-fold-down"
                viewBox="0 0 14 16"
                version="1.1"
                width="14"
            ...
            >
              ...
              >{{ item.code.replace(/^\+/, " ").replace(/^-/, " ") }}</BlobCode
            >
          </div>

          <div v-if="showComments">
            <ReviewCommentGroup
              class="review-comment-wrapper"
              v-for="rootReviewCommentItem in getRootReviewComments(index)"
              :key="rootReviewCommentItem.id"
              :rootReviewComment="rootReviewCommentItem"
            ></ReviewCommentGroup>
          </div>
          ...
          <SingleCommentCreator
            :path="file.filename"
            :position="index"
            v-if="showSingleCommentCreatorAt.some((i) => i == index)"
            @cancel="() => triggerShowSingleCommentCreator(index, false)"
          ></SingleCommentCreator>
        </CodeLine>
      </div>
    </DiffView>
...
export default {
  ...
  computed: {
    ...mapState({
      state_newCreatedReviewComments: (state) =>
        state.pullRequestDetail.newCreatedReviewComments,
      state_deletedReviewComments: (state) =>
        state.pullRequestDetail.deletedReviewComments,
    }),
  ...
  methods: {
    getRootReviewComments(index) {
      return (
        this.rootReviewCommentsHolder[index] &&
        this.rootReviewCommentsHolder[index].sort(
