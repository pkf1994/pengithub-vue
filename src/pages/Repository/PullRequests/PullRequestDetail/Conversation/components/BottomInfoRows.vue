<template>
    <BottomInfoRows v-if="data" :data="data">
        <div class="info-bottom-item">
            <span v-if="viewerCanManageIssue()" @click="() => showModal('chooseReviewerModal')" class="float-right">
            ...
            <div class="info-bottom-item-title">
                Reviewers
            </div>
         <!--    <div style="margin-bottom:10px" v-for="item in data.requested_reviewers.slice(0,21)" :key="item.id">
             ...
             <div style="margin-bottom:10px" v-for="item in relativeReviewers.slice(0,21)" :key="item.author.id">
                <router-link :to="`/${item.author.login}`">
                    <ImgWrapper class="avatar avatar-user">
                        <img class="avatar avatar-user" :src="item.author.avatarUrl" width="20" height="20" :alt="`@${item.login}`"> 
                    </ImgWrapper>
                    <span class="assignee-login">{{item.author.login}}</span>    
                </router-link> 
                <router-link class="float-right" :to="`/${owner}/${repo}/pulls/${number}/files/${item.commit.oid}`">
                    <svg v-if="item.state.toLowerCase() == 'commented'" class="octicon octicon-comment text-gray" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path fill-rule="evenodd" d="M2.75 2.5a.25.25 0 00-.25.25v7.5c0 .138.112.25.25.25h2a.75.75 0 01.75.75v2.19l2.72-2.72a.75.75 0 01.53-.22h4.5a.25.25 0 00.25-.25v-7.5a.25.25 0 00-.25-.25H2.75zM1 2.75C1 1.784 1.784 1 2.75 1h10.5c.966 0 1.75.784 1.75 1.75v7.5A1.75 1.75 0 0113.25 12H9.06l-2.573 2.573A1.457 1.457 0 014 13.543V12H2.75A1.75 1.75 0 011 10.25v-7.5z"></path></svg>
                    <svg v-if="item.state.toLowerCase() == 'changes_requested'" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path fill-rule="evenodd" d="M2.75 1.5a.25.25 0 00-.25.25v12.5c0 .138.112.25.25.25h10.5a.25.25 0 00.25-.25V4.664a.25.25 0 00-.073-.177l-2.914-2.914a.25.25 0 00-.177-.073H2.75zM1 1.75C1 .784 1.784 0 2.75 0h7.586c.464 0 .909.184 1.237.513l2.914 2.914c.329.328.513.773.513 1.237v9.586A1.75 1.75 0 0113.25 16H2.75A1.75 1.75 0 011 14.25V1.75zm7 1.5a.75.75 0 01.75.75v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5a.75.75 0 01-1.5 0V7h-1.5a.75.75 0 010-1.5h1.5V4A.75.75 0 018 3.25zm-3 8a.75.75 0 01.75-.75h4.5a.75.75 0 010 1.5h-4.5a.75.75 0 01-.75-.75z"></path></svg>
                    <svg v-if="item.state.toLowerCase() == 'approved'" class="octicon octicon-check text-green" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path fill-rule="evenodd" d="M13.78 4.22a.75.75 0 010 1.06l-7.25 7.25a.75.75 0 01-1.06 0L2.22 9.28a.75.75 0 011.06-1.06L6 10.94l6.72-6.72a.75.75 0 011.06 0z"></path></svg>
                </router-link>    
            </div> 
...
export default {
    ...
    methods: {
        network_getData() {
            if(this.accessToken) {
                this.network_getDataFromGraphql()
            }else{
                this.network_getDataFromRest()
            }
        },
    ...
    watch: {
        'data.node_id':function(newOne) {
            if(newOne) {
                if(this.accessToken)  this.network_getDataFromGraphql()
            }
        }
    },
    components: {
