<template>
    <CommonLoadingWrapper class="bg-white flex-grow-1 flex-column" 
        ...
        <AnimatedHeightWrapper :inactivatedFlagSignal="countByState != undefined">
            <Statistic class="px-3 pb-3 statistic" v-if="countByState" :class="{disabled:countByState.loading}">
                <router-link :to="countByState.toOpen" active-class="no-active-style" :class="{active: countByState.currentIssueState === 'open'}">
                    ...
                    &nbsp;{{thousands(countByState.open)}} Open
                </router-link> 
                &nbsp;
                <router-link :to="countByState.toClosed" active-class="no-active-style" :class="{active: countByState.currentIssueState === 'closed'}">
                    ...
                    &nbsp;{{thousands(countByState.closed)}} Closed
                </router-link> 
            </Statistic>
        </AnimatedHeightWrapper>
        ...
        <Entries class="flex-grow-1">
            ...
            <EntriesContent class="flex-column flex-grow-1">
                <Skeleton v-if="loading && data.length == 0">
                    <IssueItemSkeleton :perPage="perPage"></IssueItemSkeleton>
                </Skeleton>
                <div v-else>
...
<script>
    ...
    export default {
        ...
        methods: {
            clickLabelHandler(labelName){
                console.log(labelName)
                this.$emit('click-label',labelName)
            },
            thousands(payload) {
                return util_numberFormat.thousands(payload)
            }
        },
        components: {
            CommonLoadingWrapper,
