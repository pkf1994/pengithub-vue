<template>
    <router-link v-if="!issueIsEmpty" :to="PRRouterLink" class="list-item container">
        ...
        <AnimatedHeightWrapper :stretch="stretchLabels">
            <Labels v-if="showLabels" class="labels relative">
                <span v-for="item in issue.labels.nodes" :meta="randomMeta"  class=" v-align-text-bottom label d-inline-block" :key="item.id" :style="{color: isLight(`#${item.color}`) ? 'black' : 'white', background: `#${item.color}`}">
                    {{item.name}}
                </span>
            </Labels>
    ...
    export default {
        ...
        computed: {
            ...
            formatClosedDate:function () {
                return util_dateFormat.getDateDiff(this.issue.closedAt)
            },
            issueIsEmpty() {
        ...
        methods: {
            ...
            adjustStyle() {
                if(!this.showLabels || !this.issue.labels.nodes || this.issue.labels.nodes.length === 0) return
                let labelElList = document.querySelectorAll(`.labels .label[meta=${this.randomMeta}]`)
